...
public class OrcBrain : MonoBehaviour {
    ...
    private string SavePath => Path.Combine(Application.persistentDataPath, "RLBrain.json");

    void Start() {
        LoadBrainFromManager();
    }
    ...
    public void SaveBrain() {
        try {
            ...
            string path = Path.Combine(Application.persistentDataPath, "RLBrain.json");
            File.WriteAllText(path, JsonUtility.ToJson(data));

            #if UNITY_WEBGL && !UNITY_EDITOR
                PlayerPrefs.Save(); 
            #endif
            Debug.Log("Zeka otomatik olarak kaydedildi.");
        } catch (System.Exception e) { Debug.LogError("Kaydetme Hatası: " + e.Message); }
    }
  
    

    public void LoadBrainFromManager() {
        if (BrainManager.Instance != null && !string.IsNullOrEmpty(BrainManager.Instance.loadedJsonData)) {
            try {
                string jsonToLoad = BrainManager.Instance.loadedJsonData;
                if (jsonToLoad == "{}" || jsonToLoad == "[]") {
                    qTable.Clear();
                    return;
                }
                QTableData data = JsonUtility.FromJson<QTableData>(jsonToLoad);
                qTable.Clear();
    ...
    public void ApplyReward(float reward) {
        if (!string.IsNullOrEmpty(lastState)) {
            UpdateQTable(lastState, GetState(), lastAction, reward);
            
            rewardCounter++;
            if (rewardCounter >= saveInterval) {
                SaveBrain();
                rewardCounter = 0;
            }
        }
...
public class QTableData {
    ...
    public List<FloatArray> values = new List<FloatArray>();
}
